...
def bpf(data:   np.ndarray,
    ...
    order
        Order of the filter
    version
        Implementation of BPF (False = butterworth, 
        True = two LPF's subtracted from each other)
    -------
    x_filt
    ...
    ''' 
    if version:
        return lpf(data, fstop, fs) - lpf(data, fstart, fs)
...
def lpf(data:   np.ndarray,
    ...
    axis
        Axis along which to filter
    padtype
        Padtype for filter to make assumptions about data
        Look at scipy.filt for padtype options (this uses 
        the same type)
