...
if __name__ == '__main__':
    ...
    DATA_DIR = r'C:\Users\ltber\Desktop\files\AFIT\Research\WPAFB_Survey\data\flight_data\sensor_health_test\Dev_Kit_Data\SRVY0_10_21_22\ACQU5'

    START_DT = dt.datetime(2022, 10, 19, 11, 0, 0, tzinfo=pytz.utc)
    START_DT = dt.datetime.fromtimestamp(START_DT.timestamp())
    ...
    plt.figure()
    plt.title('Dev Kit Scalar Head 1')
    plt.scatter(df.datetime, df.SCALAR_1, s=1)
    plt.xlabel('Datetime (UTC)')
    plt.ylabel('Nagnetic Intensity (nT)')
    plt.grid()
    
    plt.figure()
    plt.title('Dev Kit Scalar Head 2')
    plt.scatter(df.datetime, df.SCALAR_2, s=1)
    plt.xlabel('Datetime (UTC)')
    plt.ylabel('Nagnetic Intensity (nT)')
    plt.grid()
