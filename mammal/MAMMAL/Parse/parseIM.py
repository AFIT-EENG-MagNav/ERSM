...
def parse_sec(fname:     str,
              fast_mode: bool=True,
              chunk:     int=1000,
              second:    bool=True) -> pd.DataFrame:
    '''
    Parse the INTERMAGNET (.sec) -- or (.min) if specified -- sensor 
    file and return a pandas data frame with the resulting parsed data
    ...
    chunk
        ...
        to simultaneously calculate IGRF values for
    second
    	Whehter or not files are .sec or .min (True => .sec)
                ...
                if id not in data.keys():
                    ...
                    data[id] = pd.concat([data[id],
                                            parse_sec(join(root, file),
                                                    fast_mode,
                                                    chunk, second=second)]).sort_values(by=['DOY', 'TIME']).dropna()
...
if __name__ == '__main__':
    df = parse_sec(r'C:\Users\ltber\Desktop\files\AFIT\Research\WPAFB_Survey\data\ground_reference\frd20220728psec.sec')
    print(df)
