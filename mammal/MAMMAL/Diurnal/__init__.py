...
def interp_reference_df(df:            pd.DataFrame,
                        ...
                        ref_scale:     float=1,
                        ref_offset:    float=0,
                        subtract_core: bool=False,
                        second:        bool=False) -> list:
    '''
    ...
    second
        Whether or not the reference data (df) being processed is second
        or minute data.
    ...
    if survey_lon is not None:
        t, f, x, y, z = longitude_norm(df, survey_lon, second=second)
    else:
...
def longitude_norm(ref_df:       pd.DataFrame,
                   survey_lon:   float,
                   corner_frq:   float=1/(3600*3),
                   second:       bool=True) -> list:
    '''
    ...
    second
        Whether or not the data being processed is second
        or minute data. If second fs = 1, or fs = 1/60
    Returns
    -------
    list
    ...
    lpf_f = Filters.lpf(f, corner_frq, fs, 1)
    hpf_f = Filters.hpf(f, corner_frq, fs, 1)
    
    lpf_x = Filters.lpf(x, corner_frq, fs, 1)
    hpf_x = Filters.hpf(x, corner_frq, fs, 1)
    
    lpf_y = Filters.lpf(y, corner_frq, fs, 1)
    hpf_y = Filters.hpf(y, corner_frq, fs, 1)
    
    lpf_z = Filters.lpf(z, corner_frq, fs, 1)
    hpf_z = Filters.hpf(z, corner_frq, fs, 1)
